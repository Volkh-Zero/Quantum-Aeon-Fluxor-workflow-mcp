...
appendToResults('🧪 Comprehensive Testing of AI Expert Workflow MCP with OpenRouter API');
...
async function consultWithExpert(role, input) {
  ...
  try {
    ...
    await appendToResults(`❌ ${role} consultation test failed: ${error.message}`);
...
async function generateExpertDocument(role, template, input) {
  ...
  try {
    ...
    await appendToResults(`❌ ${role} document generation test failed: ${error.message}`);
    return { success: false, error: error.message };
  }
}
...
async function testErrorHandling() {
  ...
  try {
    ...
    await appendToResults(`❌ Error handling test failed: ${error.message}`);
    return false;
  }
}
...
`;
  ...
  console.log(`\n📝 Test results saved to tests/results/result_test.md`);
